...
db_klasse= "klasse_5_d" #Default test table
...
#hadles sql querys with no return value
...
#hadles sql querys with one return value
def _send_query_one(query):
    ...
    try:
        ...
        cursor.close()
        conn.close()
#hadles sql querys with list return value
...
def insert_row(table, fach, fach_amount, name, email):
    query = "INSERT INTO " + table + "(name,email,fach_"+ str(fach) +"," + int_to_fach_amount(fach)+") " \
            "VALUES(%s,%s,%s,%s)" % name, email, "0", str(fach_amount)
    try:
